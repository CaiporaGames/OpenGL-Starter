...
class OrthoCamera2D {
public:
    void setViewport(int fbw, int fbh) { fbw_ = std::max(1, fbw); fbh_ = std::max(1, fbh); }
    void setCenter(const glm::vec2& c) { center_ = c; }
    glm::vec2 center() const { return center_; }

    void setHeightWorld(float h) { height_ = std::clamp(h, minH_, maxH_); }
    float heightWorld() const { return height_; }
